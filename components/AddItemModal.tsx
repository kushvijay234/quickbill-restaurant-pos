...
const AddItemModal: React.FC<AddItemModalProps> = ({ onClose, onAddItem }) => {
  ...
  const [variants, setVariants] = useState<{name: string, price: string}[]>([{ name: 'Full', price: '' }]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!name || !imageUrl) {
      setError('Item Name and Image URL are required.');
      return;
    }
    ...
    onAddItem({ name, imageUrl, variants: processedVariants });
  };
