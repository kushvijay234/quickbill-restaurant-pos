...
const BillModal: React.FC<BillModalProps> = ({ order, profile, onClose }) => {
  ...
  const handleShareOnWhatsApp = () => {
    ...
    const itemsText = orderItems.map(
        ...
        `${item.name} (${selectedVariant.name}) x ${quantity} - ${currency.symbol}${(selectedVariant.price * quantity * currency.rate).toFixed(2)}`
    ).join('\n');
...
Thank you for your visit!
    ...
    const encodedText = encodeURIComponent(billText.trim().replace(/\n\s*\n/g, '\n\n'));
    // Use a URL that works on both mobile and desktop
    const whatsappUrl = `https://wa.me/${customer.mobile.replace(/\D/g, '')}?text=${encodedText}`;
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl p-8 w-full max-w-sm m-4 transition-colors duration-300">
        <div id="bill-modal" ref={billRef} className="bg-white dark:bg-gray-800 p-4 rounded-md">
        ...
        <div className="no-print mt-8 flex flex-col sm:flex-row justify-end gap-3">
          <button onClick={onClose} className="bg-gray-200 text-gray-800 px-4 py-2 rounded-md hover:bg-gray-300 dark:bg-gray-600 dark:text-gray-200 dark:hover:bg-gray-500 font-medium w-full sm:w-auto">
            Close
          </button>
          <button
            onClick={handleShareOnWhatsApp}
            disabled={!customer.mobile}
            title={!customer.mobile ? "Add a customer mobile number to share" : "Share on WhatsApp"}
          ...
          <button onClick={handlePrint} className="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700 font-medium w-full sm:w-auto">
            Print
          </button>
        </div>
      </div>
