...
const EditItemModal: React.FC<EditItemModalProps> = ({ item, onClose, onUpdateItem }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setError('');
    onUpdateItem({ ...item, name, price: priceNumber });
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl p-8 w-full max-w-md m-4 transition-colors duration-300">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label htmlFor="itemPrice" className="block text-sm font-medium text-gray-700 dark:text-gray-300">New Price (in INR)</label>
            <input
              type="number"
              id="itemPrice"
              value={price}
